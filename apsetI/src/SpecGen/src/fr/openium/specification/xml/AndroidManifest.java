...
public final class AndroidManifest {
	...
	public final static String ATTRIBUTE_PERMISSION = "permission";
	public final static String ATTRIBUTE_HOST = "host";
	public final static String ATTRIBUTE_MIMETYPE = "mimeType";
	public final static String ATTRIBUTE_PATH = "path";
	public final static String ATTRIBUTE_SCHEME = "scheme";
	private static final String ANDROID_MANIFEST_XML = "AndroidManifest.xml";
	public final static String ATTRIBUTE_AUTHORITIES = "authorities";
	...
	public static IAbstractFile getManifest(IAbstractFolder projectFolder) {
		IAbstractFile file = projectFolder.getFile(ANDROID_MANIFEST_XML);
		if (file.exists()) {
			return file;
		}
	...
	public static String getPackage(IAbstractFile manifestFile)
		...
		return xPath.evaluate("/" + NODE_MANIFEST + "/@" + ATTRIBUTE_PACKAGE,
	 ...
	 * 
	 * If the give manifest does not contain the debuggable attribute then the
	 * application is considered to not be debuggable.
	 * 
	 * @param manifestFile
	 *            the manifest to parse.
	...
	public static boolean getDebuggable(IAbstractFile manifestFile)
		...
		String value = xPath.evaluate("/" + NODE_MANIFEST + "/"
				+ NODE_APPLICATION + "/@"
	...
	/**
	 * Returns the value of the versionCode attribute or -1 if the value is not
	 * set.
	 * 
	 * @param manifestFile
	...
	public static int getVersionCode(IAbstractFile manifestFile)
		...
		String result = xPath.evaluate("/" + NODE_MANIFEST + "/@"
				+ AndroidXPathFactory.DEFAULT_NS_PREFIX + ":"
	 ...
	 * @return true if the versionCode attribute is present and its value is not
	 *         empty.
	 * @throws XPathExpressionException
	 * @throws StreamException
	 *             If any error happens when reading the manifest.
	...
	public static boolean hasVersionCode(IAbstractFile manifestFile)
		...
		Object result = xPath.evaluate("/" + NODE_MANIFEST + "/@"
				+ AndroidXPathFactory.DEFAULT_NS_PREFIX + ":"
				+ ATTRIBUTE_VERSIONCODE,
				new InputSource(manifestFile.getContents()),
				XPathConstants.NODE);
	 ...
	 * <p/>
	 * If the attribute is set with an int value, the method returns an Integer
	 * object.
	 * <p/>
	 * If the attribute is set with a codename, it returns the codename as a
	 * String object.
	 * <p/>
	...
	public static Object getMinSdkVersion(IAbstractFile manifestFile)
		...
		String result = xPath.evaluate("/" + NODE_MANIFEST + "/"
				+ NODE_USES_SDK + "/@" + AndroidXPathFactory.DEFAULT_NS_PREFIX
	...
	/**
	 * Returns the value of the targetSdkVersion attribute (defaults to 1 if the
	 * attribute is not set), or -1 if the value is a codename.
	 * 
	 * @param manifestFile
	 *            the manifest file to read the attribute from.
	 * @return the integer value or -1 if not set.
	...
	public static Integer getTargetSdkVersion(IAbstractFile manifestFile)
		...
		String result = xPath.evaluate("/" + NODE_MANIFEST + "/"
				+ NODE_USES_SDK + "/@" + AndroidXPathFactory.DEFAULT_NS_PREFIX
				+ ":" + ATTRIBUTE_TARGET_SDK_VERSION, new InputSource(
	...
	public static String getApplicationIcon(IAbstractFile manifestFile)
		...
		return xPath.evaluate("/" + NODE_MANIFEST + "/" + NODE_APPLICATION
				+ "/@" + AndroidXPathFactory.DEFAULT_NS_PREFIX + ":"
				+ ATTRIBUTE_ICON, new InputSource(manifestFile.getContents()));
	}
	...
	public static String getApplicationLabel(IAbstractFile manifestFile)
		...
		return xPath.evaluate("/" + NODE_MANIFEST + "/" + NODE_APPLICATION
				+ "/@" + AndroidXPathFactory.DEFAULT_NS_PREFIX + ":"
				+ ATTRIBUTE_LABEL, new InputSource(manifestFile.getContents()));
	}

	/**
	 * Combines a java package, with a class value from the manifest to make a
	 * fully qualified class name
	 * 
	 * @param javaPackage
	 *            the java package from the manifest.
	 * @param className
	 *            the class name from the manifest.
	 * @return the fully qualified class name.
	 */
	public static String combinePackageAndClassName(String javaPackage,
			String className) {
	...
	/**
	 * Given a fully qualified activity name (e.g. com.foo.test.MyClass) and
	 * given a project package base name (e.g. com.foo), returns the relative
	 * activity name that would be used the "name" attribute of an "activity"
	 * element.
	 * 
	 * @param fullActivityName
	 *            a fully qualified activity class name, e.g.
	 *            "com.foo.test.MyClass"
	 * @param packageName
	 *            The project base package name, e.g. "com.foo"
	 * @return The relative activity name if it can be computed or the original
	 *         fullActivityName.
	 */
	public static String extractActivityName(String fullActivityName,
			String packageName) {
		if (packageName != null && fullActivityName != null) {
			if (packageName.length() > 0
					&& fullActivityName.startsWith(packageName)) {
