...
/**
 * 
 * @author Stassia generate Junit code for integrity vulnerability
 * 
 */
public class TcGenerationToolIntegrity extends AbstractTcGenerationTool {

	public TcGenerationToolIntegrity() {
		super();
	...
	ArrayList<String> extra_key = new ArrayList<String>();
	private String mAction = "";
	private boolean istestProjectNameWritten = false;
	...
	private void writeActivityTest(IostsConcreteTestCase concreteTc,
		...
		if (mManifestData.getProviders().size() != 0) {
			...
			add(END);
			add(CATCH);
			add(" (IllegalArgumentException e2) {");
			addln();
	...
	private String random(ArrayList<String> sQL_Injections) {
		Random rd = new Random();
		int randomInt = rd.nextInt(sQL_Injections.size());
		return sQL_Injections.get(randomInt);
	}
	...
	private void setOutpuAssert(IostsConcreteTestCase concreteTC,
			String columnName, String mContentvalues) {
		/**
		 * Princip:
		 */
		/**
		...
		for (int i = 0; i < passOutput.size(); i++) {
			...
			if (passOutput.get(i) instanceof String) {
				...
				// if (!inTry.toString().contains(TRY))
				// tryAndCatch(inTry);
